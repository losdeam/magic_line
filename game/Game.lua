...
function Game.prototype.enemyAct(self) -- 188
	...
	if action == "charge" then -- 190
		self.hud:showNotice("精英蓄力中 — 下次行动重击", 0, 16757611) -- 193
	elseif action == "heavy" then -- 193
		self.hud:hitPlayer(damage) -- 195
		self.hud:showNotice( -- 196
			("精英重击 −" .. tostring(damage)) .. "，棋盘被封锁", -- 196
			0, -- 196
			16747146 -- 196
		) -- 196
	else -- 196
		self.hud:hitPlayer(damage) -- 198
		self.hud:showNotice("敌方出手", 2, 16757611) -- 199
	end -- 199
	if self.combat.lastExpiredLocks > 0 then -- 199
		self.hud:showNotice( -- 202
			("封锁自动解除 " .. tostring(self.combat.lastExpiredLocks)) .. " 格", -- 202
			2, -- 202
			10475263 -- 202
		) -- 202
	end -- 202
	self.view:refreshFromBoard() -- 205
	self.turnsUntilEnemy = self.combat.enemyInterval -- 206
	if self.combat.player.hp <= 0 then -- 206
		self:defeat() -- 208
		return -- 209
	end -- 209
	self:refreshHud() -- 211
end -- 188
function Game.prototype.handleSkill(self, id) -- 214
	if self.defeated then -- 214
		return -- 216
	end -- 216
	local before = self:captureBlocks() -- 218
	local result = self.combat:useSkill(id) -- 219
	self.hud:showNotice(result.message, 2, result.ok and 9427199 or 16757611) -- 220
	if result.ok then -- 220
		self.hud:glowSkill(id) -- 223
		local changed = self:diffBlocks(before) -- 224
		local resetNotice = self.combat:ensureBoardPlayable() -- 225
		if resetNotice ~= nil then -- 225
			self.hud:showNotice(resetNotice) -- 227
		end -- 227
		self.view:refreshFromBoard() -- 229
		self.view:skillCast(changed) -- 230
	end -- 230
	self:refreshHud() -- 232
end -- 214
function Game.prototype.captureBlocks(self) -- 236
	local snapshot = {} -- 237
	do -- 237
		local row = 0 -- 238
		while row < self.board.rows do -- 238
			do -- 238
				local col = 0 -- 239
				while col < self.board.columns do -- 239
					snapshot[#snapshot + 1] = self.board:cellIndex(col, row) -- 240
					col = col + 1 -- 239
				end -- 239
			end -- 239
			row = row + 1 -- 238
...
function Game.prototype.diffBlocks(self, before) -- 247
	local changed = {} -- 248
	local i = 0 -- 249
	do -- 249
		local row = 0 -- 250
		while row < self.board.rows do -- 250
			do -- 250
				local col = 0 -- 251
				while col < self.board.columns do -- 251
					local index = self.board:cellIndex(col, row) -- 252
					if i < #before and before[i + 1] ~= index then -- 252
						changed[#changed + 1] = self.board:flatIndex(col, row) -- 254
					end -- 254
					i = i + 1 -- 256
					col = col + 1 -- 251
				end -- 251
...
function Game.prototype.toggleMode(self) -- 263
	local mode = self.settings:toggleMode() -- 264
	local saved = self.settings:save() -- 265
	self.combat:setRealtime(mode == "realtime") -- 266
	self:restart() -- 268
	self.hud:showNotice(("已切换为" .. self.settings:modeName()) .. (saved and "" or "（设置未保存）")) -- 269
	self:refreshHud() -- 270
end -- 263
function Game.prototype.cycleDifficulty(self) -- 274
	self.settings:cycleDifficulty() -- 275
	local saved = self.settings:save() -- 276
	self.combat:setDifficultyScale(self.settings:difficultyScale()) -- 277
	self.combat:loadWave() -- 278
	self.hud:showNotice(("难度：" .. self.settings:difficultyName()) .. (saved and "" or "（设置未保存）")) -- 279
	self:refreshHud() -- 280
end -- 274
function Game.prototype.toggleHint(self) -- 284
	self.settings:toggleHint() -- 285
	local saved = self.settings:save() -- 286
	self.hud:showNotice(("操作提示：" .. (self.settings.showHint and "开" or "关")) .. (saved and "" or "（设置未保存）")) -- 287
	self:refreshHud() -- 288
end -- 284
function Game.prototype.defeat(self) -- 291
	self.defeated = true -- 292
	self.hud:showDefeat(((("玩家生命归零 — 止步于第 " .. tostring(self.combat.stageNumber)) .. " 关第 ") .. tostring(self.combat.waveNumber)) .. " 波") -- 293
	self:refreshHud() -- 294
end -- 291
function Game.prototype.restart(self) -- 297
	self.defeated = false -- 298
	self.board:reset() -- 299
	self.view:refreshFromBoard() -- 300
	self.combat:resetBattle() -- 301
	self.combat:setDifficultyScale(self.settings:difficultyScale()) -- 302
	self.combat:setRealtime(self.settings.mode == "realtime") -- 303
	self.turnsUntilEnemy = self.combat.enemyInterval -- 304
	self.hud:hideDefeat() -- 305
	self.hud:showNotice("重新开始本关") -- 306
	self:refreshHud() -- 307
end -- 297
function Game.prototype.refreshHud(self) -- 310
	local wave = self.combat:currentWave() -- 311
	self.hud:setStageInfo((((("第 " .. tostring(self.combat.stageNumber)) .. " 关 · 波次 ") .. tostring(self.combat.waveNumber)) .. "/") .. tostring(self.combat.waveCount)) -- 312
	self.hud:setEnemyInfo(wave.name, self.combat.enemy.hp, self.combat.enemy.maxHp) -- 313
	self.hud:setPlayerInfo(self.combat.player.hp, self.combat.player.maxHp, self.combat.player.mana, self.combat.player.maxMana) -- 314
	if self.combat.isRealtime then -- 314
		self.hud:setTimerInfo( -- 316
			("敌方行动倒计时 " .. tostring(math.ceil(self.combat.secondsLeft))) .. " 秒", -- 317
			self.combat.secondsLeft / self.combat.enemyIntervalSeconds -- 318
		) -- 318
	else -- 318
		self.hud:setTimerInfo( -- 321
			("敌方行动倒计时 " .. tostring(self.turnsUntilEnemy)) .. " 回合", -- 322
			self.turnsUntilEnemy / self.combat.enemyInterval -- 323
		) -- 323
	end -- 323
	self.hud:syncSkills(self.combat.player.mana) -- 326
	self.hud:syncSettings( -- 327
		self.settings:modeName(), -- 327
		self.settings:difficultyName(), -- 327
		self.settings.showHint, -- 327
		self.settings.saved -- 327
	) -- 327
end -- 310
